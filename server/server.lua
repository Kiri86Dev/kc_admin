...
function isBan (identifier)
    ...
    local isban_mysql = MySQL.Sync.fetchAll("SELECT * FROM kc_bans WHERE identifier = @identifier", {["@identifier"] = identifier})
...
AddEventHandler("kc_admin:check_jail", function()
    ...
    local mysql_jails = MySQL.Sync.fetchAll("SELECT * FROM kc_jails WHERE identifier = @identifier", {["@identifier"] = identifier})
...
AddEventHandler("kc_admin:unjail", function (target_id, force)
    ...
    if force then
        if xPlayer.getGroup() == 'admin' then
            MySQL.Async.execute("DELETE FROM kc_jails WHERE identifier = @identifier", {["@identifier"] = identifier}, function (rows)
                if rows == 1 then
    ...
    local jail_time_sql = MySQL.Sync.fetchAll("SELECT time FROM kc_jails WHERE identifier = @identifier", {["@identifier"] = identifier})
    ...
    if getTime() >= jail_time then
        local unjail_sql = MySQL.Sync.execute("DELETE FROM kc_jails WHERE identifier = @identifier", {["@identifier"] = identifier})
        TriggerClientEvent("kc_admin:unjail_player", target_id)
    end
end)
...
AddEventHandler("kc_admin:jail", function (target_id, time)
    ...
    MySQL.Async.execute("INSERT INTO kc_jails (identifier, name, admin_name, admin_identifier, time, time_s) VALUES (@identifier, @name, @admin_name, @admin_identifier, @timestamp, @time)", {["@identifier"] = identifier, ["@name"] = user_name, ["@admin_name"] = admin_name, ["@admin_identifier"] = admin_identifier, ["@timestamp"] = timestamp, ["@time"] = time}, function(rows)
        if rows == 1 then
...
AddEventHandler("kc_admin:delete_ban", function (ban_id)
    ...
    MySQL.Async.execute("DELETE FROM kc_bans WHERE id=@id", {["@id"] = ban_id}, function (rows)
        if rows == 1 then
...
AddEventHandler("kc_admin:get_warns", function (id) 
    ...
    local identifier = getIdentifier(id)
    local result = MySQL.Sync.fetchAll("SELECT id, reason, admin_name, timestamp FROM kc_warns WHERE identifier=@identifier", {["@identifier"] = identifier})
